...
def update_vins():
    ...
    # Trouver l'ID de la catégorie Vins
    cursor.execute("SELECT id FROM categories WHERE nom = 'Vins'")
    result = cursor.fetchone()
    
    if not result:
        print("❌ Catégorie Vins non trouvée")
        conn.close()
        return
    
    categorie_id = result[0]
    print(f"ID catégorie Vins: {categorie_id}")
    
    # Supprimer les anciens vins
    print("Suppression des anciens vins...")
    cursor.execute("DELETE FROM produits WHERE categorie_id = ?", (categorie_id,))
    
    # Importer les nouveaux vins
    print("Importation des nouveaux vins...")
    ordre = 1
    for nom, stock in vins_corrects:
        cursor.execute('''
    ...
    print(f"✅ {count} vins importés avec succès !")
    conn.close()
